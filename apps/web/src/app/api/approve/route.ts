...
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json(
      { 
        status: "fail", 
        msg: "approve requires server-side private key. Use client-side transaction signing instead." 
      },
      { status: 400 }
    );
    ...
    // Get tx options (simplified - would need gas station integration)
    const txOptions = { gasLimit: "5000000" };
    const dApp = platform as Dapp;
    const tx = await pool.approve(dApp, body.asset, ethers.constants.MaxUint256, txOptions);
